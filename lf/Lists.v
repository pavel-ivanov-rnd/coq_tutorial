...
Definition member (v : nat) (s : bag) : bool :=
  if 1 <=? count v s then true else false.
...
Lemma a_is_a: forall a, (a =? a) = true.
Proof. intros. induction a. reflexivity. 
  simpl. rewrite IHa. reflexivity.
Qed.
Theorem bag_theorem : forall b a, count a (b ++ [a]) = count a b + 1.
Proof.
  intros b a. induction b. simpl. rewrite a_is_a. reflexivity.
  simpl. rewrite IHb.
Admitted.
...
Lemma count_distr: forall (a : bag) (b : bag) (n : nat),
  count n (a ++ b) = count n a + count n b.
...
Theorem count_member_nonzero : forall (s : bag),
  1 <=? (count 1 (1 :: s)) = true.
Proof.
  intros. assert (1 :: s = [1] ++ s). reflexivity. rewrite H. induction s. reflexivity.
  assert ([1] ++ n :: s = [1] ++ [n] ++ s). reflexivity.
  rewrite H0.  rewrite count_distr. simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros. induction s. reflexivity. destruct n. simpl. 
  assert (forall a : nat, a <=? a + 1 = true).
    intros. induction a. reflexivity. 
    simpl. rewrite <- IHa. reflexivity.
    rewrite H. reflexivity.
  simpl. rewrite <- IHs. 
  assert (forall a : nat, a + 0 = a). 
    induction a. reflexivity. simpl. rewrite IHa. reflexivity.
...
Proof.
  ...
  assert (l1 = rev(rev l1)). rewrite rev_involutive; reflexivity.
